...
<div id="header"> <a href="index.php" id="logo"><img src="images/logo.png" alt=""></a>
        <?php
            ...
            //Logika Harga
            $Jam     = $_POST['Waktu_Booking'];
            $Tanggal = $_POST['Tanggal_Booking'];
            $Harian  = getWeekday($Tanggal);

            debug_to_console("Jam",$Jam);
            debug_to_console("Tanggal",$Tanggal);
            debug_to_console("Harian",$Harian);


            if ($Harian == 1) {
              debug_to_console("Hari","Senin");
              if(strtotime($Jam)<=strtotime('17:00:00')) {
                  debug_to_console("Room_Price","Normal");
                  $Room_Price = $RoomRow["Room_Price"];
              } else {
                  debug_to_console("Room_Price","Bussines");
                  $Room_Price = $RoomRow["Room_Business_Price"];
              }
              # code...
            } elseif ($Harian == 2) {
              debug_to_console("Hari","Selasa");
              if(strtotime($Jam)<=strtotime('17:00:00')) {
                  debug_to_console("Room_Price","Normal");
                  $Room_Price = $RoomRow["Room_Price"];
              } else {
              ...
              debug_to_console("Hari","Sabtu");
              if(strtotime($Jam)<=strtotime('17:00:00')) {
                  debug_to_console("Room_Price","Normal");
                  $Room_Price = $RoomRow["Room_Price"];
              } else {
                  debug_to_console("Room_Price","Bussines");
                  $Room_Price = $RoomRow["Room_SpecialPrice"];
              }
            } elseif ($Harian == 0) {
              debug_to_console("Hari","Minggu");
              if(strtotime($Jam)<=strtotime('17:00:00')) {
                  debug_to_console("Room_Price","Normal");
                  $Room_Price = $RoomRow["Room_Price"];
              } else {
...
<div id="id02" class="modal">
  <form class="modal-content" method="GET" action="BookingSend.php">
    <div class="container">
